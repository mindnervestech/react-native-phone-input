...
class PhoneNumber {
  ...
  getAllCountries(country) {
    return Country.getAll(country);
  }

  getDialCode(number, country) {
    let dialCode = '';
    ...
    if (number.charAt(0) === '+') {
      ...
      for (let i = 0; i < number.length; i++) {
        ...
        if (this.isNumeric(c)) {
          ...
          if (Country.getCountryCodes(country)[numericChars]) {
  ...
  getCountryCodeOfNumber(number, country) {
    const dialCode = this.getDialCode(number);
    const numeric = this.getNumeric(dialCode);
    const countryCode = Country.getCountryCodes(country)[numeric];
  ...
  getCountryDataByCode(iso2, country) {
    return Country.getCountryDataByCode(iso2, country);
  }
}
